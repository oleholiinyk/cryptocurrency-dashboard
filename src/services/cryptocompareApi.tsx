...
export const cryptocompareApi = createApi({
  ...
  endpoints: (builder) => ({
    getDailyOHLCV: builder.query<DailyOHLCV, string>({
      query: (symbol) =>
        createRequest(`/data/v2/histoday?fsym=${symbol}&tsym=USD&limit=180`),
    }),
    getStats: builder.query({
      query: (symbol) =>
        createRequest(`/data/pricemultifull?fsyms=${symbol}&tsyms=USD`),
    }),
    getTradingSignals: builder.query({
      query: (symbol) =>
        createRequest(`/data/tradingsignals/intotheblock/latest?fsym=${symbol}`),
    }),
    getNews: builder.query({
      query: () => createRequest("/data/v2/news/?lang=EN"),
    }),
...
export const GetDailyOHLCV = (
  ...
  return (dispatch: any) => {
    axios
      .get<DailyOHLCV>(
        `${process.env.REACT_APP_CRYPTO_COMPARE_URI}/data/v2/${history}?fsym=${symbol}&tsym=USD&limit=${period}`
      )
